...
To perform a stats query on the order resource send a `POST` request to the `/orders/stats` endpoint specifying in the payload the [query keys](stats.md#query-keys) and [filter](filters.md) parameters like in the generic example below:
